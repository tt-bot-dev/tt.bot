...
/**
 * @param {import("sosamba").InteractionContext} ctx 
 * @param {Partial<import("@tt-bot-dev/types").GuildConfig>} config 
 * 
 * @return {Promise<import("@tt-bot-dev/types").GuildConfig>}
 */
...
export function getUserLocale(ctx) {
    return getUserProfile(ctx).then(p => {
        if (!p.locale) {
            if (ctx.guild) return getGuildConfig(ctx).then(g => g?.locale || "en");
            else return "en";
...
/**
 * @param {import("sosamba").InteractionContext} ctx
 * @param {keyof import("../languages/en.json")} name 
 * @param  {...any} args 
