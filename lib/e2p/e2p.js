...
const generate = async (emoji, asGif = false) => {
    const getEmojiLink = emoji => new Promise(rs => {
        if (emoji) {
            ...
            if (emojiRegexMatch && emojiRegexMatch.length > 0) {
                ...
                rs({ type: "unicodeEmote", url: `https://twitter.github.io/twemoji/2/72x72/${convertSurrogates(emoji)}.png` });
            }
        }
    });
    ...
    let data = await Promise.all(resolvedEmojis.map(r => {
        ...
        return getAndCache(r.url)
            ...
            .then(i => prepareFrames(r, apngPlugin.ImageFrame.createFromJIMP({
                bitmap: i
            })));
    }));
    resolvedEmojis = null;
    ...
    const image = asGif ? await encodeGIF(totalFrames.map(i => i.bitmap)) : await encodeAPNG(totalFrames.map(i => i.bitmap));
