...
import * as dataEncryption from "./dataEncryption.mjs";
...
class TTBotClient extends Client {
    ...
    escapeMarkdown(string) {
        ...
        const replacedBrackets = replacedUnderscores.replace(/\[/g, "\\[").replace(/\(/, "\\(")
            .replace(/\]/g, "\\]")
            .replace(/\)/g, "\\)");
    ...
    parseMsg(string, m, g) {
        ...
        const regex = new RegExp(Object.keys(replacers).map(t => t.replace(/\./g, "\\."))
            .join("|"), "gi");
