...
class TTBotClient extends Client {
    ...
    passesRoleHierarchy(member1, member2) {
        if (member1.guild.id !== member2.guild.id) throw new TypeError("Members aren't in the same guild");
        ...
        const member1Roles = member1.roles.map(r => member1.guild.roles.get(r))
            .sort((a, b) => b.position - a.position);
        const member2Roles = member2.roles.map(r => member2.guild.roles.get(r))
            .sort((a, b) => b.position - a.position);
        return member1Roles[0].position > member2Roles[0].position;
    }
    async postStats() {
        if (!config.dbotskey) return;
    ...
    async isModerator(member, botOwnerIsMod = true) {
        ...
        if (server) {
            const role = member.guild.roles.find(r => r.name.toLowerCase() === server.modRole.toLowerCase());
            if (role) { serverHasModRole = true; modRole = role; }
        } else {
            const role = member.guild.roles.find(r => r.name.toLowerCase() === "tt.bot mod");
            if (role) { serverHasModRole = true; modRole = role; }
        }
