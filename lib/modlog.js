...
class ModLog {
    ...
    async getUserStrikes(userID, ctx) {
        ...
        let guildStrikes = await ctx.db.table("modlog").get(guildID);
        if (!guildStrikes) guildStrikes = await this.insertNew(guildID, ctx);
        let {items} = guildStrikes;
        return items.filter(s => s.type === PunishTypes.STRIKE && s.userID === userID);
    }

    async insertNew(guildID, ctx) {
        await ctx.db.table("modlog").insert({
            id: guildID,
            items: []
        });
        return await ctx.db.table("modlog").get(guildID);
    }
