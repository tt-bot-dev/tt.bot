...
module.exports = sosamba => class RethinkDBProvider extends DBProvider {
    constructor(opts) {
        ...
        this._dbLock = this.db.db(opts.db || this.db._db).wait()
            .then(() => this.databaseSetup());
    ...
    async updateModlog(guildID, data) {
        await this._dbLock;
        await this.db.table("modlog").get(guildID)
            .update(data);
    }
    ...
    async getBlacklistedGuildById(id) {
        await this._dbLock;
        return await this.db.table("blacklist")
            .filter(this.db.row("id").eq(id)
                .or(this.db.row("ownerID").eq(id)));
    }
