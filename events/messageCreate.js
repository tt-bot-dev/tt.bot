module.exports = async function (msg) {
    ...
    if (msg.content.startsWith(server.prefix)|| msg.content.startsWith(config.prefix)) { // if the content starts with the prefix
        let nameslice = msg.content.startsWith(config.prefix) ? msg.content.slice(config.prefix.length) : msg.content.slice(server.prefix.length); // we slice it so we can get the command
