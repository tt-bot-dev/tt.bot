...
class LockCommand extends Command {
    constructor(sosamba, ...args) {
        super(sosamba, ...args, {
            name: "lock",
            description: "Unlocks a channel by allowing all users from posting in the channel",
    ...
    async run(ctx, [ channel, reason ]) {
        ...
        if (channel.permissionsOf(ctx.author.id).has("sendMessages")) {
            await channel.createMessage({
                embed: {
                    ...
                    description: reason || "You can chat again as usual.",
                    color: 0xFF0000,
                    footer: {
                        text: `Unlocked by ${this.sosamba.getTag(ctx.author)}`,
        ...
        this.sosamba.modLog.createPunishment(ctx, PunishTypes.UNLOCK, undefined, reason).catch(() => void 0);
        await ctx.send(`:ok_hand: Unlocked ${channel.mention}`);
    }
