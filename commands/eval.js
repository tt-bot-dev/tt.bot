module.exports = {
    exec: async function (msg, args) {
        if (isO(msg)) {
            ...
            let data = `\`\`\`js\n${overall.replace(new RegExp(`${bot.token}|${config.token}|${config.dbots2key}|${config.dbotskey}|${config.clientSecret}|${config.novokey}|${config.connectOpts.password}`, "g"), "jako rilý?")}\n\`\`\``;
            if (data.length > 2048) {
                let gist;
                try {
